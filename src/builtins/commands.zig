...
pub const slash_specs = [_]SlashSpec{
    ...
    .{ .kind = .tree, .command = "/tree", .help_entry = "/tree [turn]", .completion_description = "list conversation turns; /tree <n> rewinds the active branch to turn n (back or forward)", .presentation_category = .session, .has_args = true, .accepts_payload = true },
    .{ .kind = .loop, .command = "/loop", .help_entry = "/loop [count|duration] [--while|--until '<cmd>'] [prompt]", .completion_description = "auto-resubmit a prompt each turn; bare /loop disables", .presentation_category = .session, .has_args = true, .accepts_payload = true },
    .{ .kind = .todo, .command = "/todo", .help_entry = "/todo", .completion_description = "show the session todo list", .presentation_category = .session },
    .{ .kind = .hub, .command = "/hub", .help_entry = "/hub [peer]", .completion_description = "list agent-hub peers and read messages; /hub <peer> shows its mailbox", .presentation_category = .session, .has_args = true, .accepts_payload = true },
