...
pub const CommandHandlers = struct {
    ...
    show_tree: *const fn (ctx: *anyopaque, rest: []const u8) anyerror!void,
    handle_loop: *const fn (ctx: *anyopaque, rest: []const u8) anyerror!void,
    show_todo: *const fn (ctx: *anyopaque) anyerror!void,
    show_hub: *const fn (ctx: *anyopaque, rest: []const u8) anyerror!void,
...
pub fn route(registry: SlashRegistry, handlers: *const CommandHandlers, cmd: []const u8) !void {
    switch (parse(registry, cmd)) {
        ...
        .tree => |rest| try handlers.show_tree(handlers.ctx, rest),
        .loop => |rest| try handlers.handle_loop(handlers.ctx, rest),
        .todo => try handlers.show_todo(handlers.ctx),
        .hub => |rest| try handlers.show_hub(handlers.ctx, rest),
...
fn testHandlers(ctx: *TestContext) CommandHandlers {
    return .{
        ...
        .show_tree = unexpectedPayload,
        .handle_loop = unexpectedPayload,
        .show_todo = unexpectedNoPayload,
        .show_hub = unexpectedPayload,
